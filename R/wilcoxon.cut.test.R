...
#' This group vector may be numerical or factorial with 2 levels. A proportion of data around the median may be possibly 
#' exclude. The two remaining samples of data in  `x`, associated  for  lower an upper parts of the group 
...
wilcoxon.cut.test <- function(x, 
  ...
  if(!is.null(data)){
      if(length(x)==1){# get variables names
        ...
        if(debug){ 
	cat('\nStructure de x (wilcoxon.cut.test) : '); 
	print(str(x))}        
        ...
        if(bootstrap==0){# calcul unique sans bootstrap
          ...
          foreach(k = 1:bootstrap, .combine=c ) %dopar% {
            ...
            wilcox.test(x=x.boot[group.boot==1],
                        y=x.boot[group.boot==2],
                        exact=FALSE)$p.value

          }-> pval
            q <- quantile(pval,c(0.5,0.95))
          ...
          return(DF) # ne renvoie pas DF ??
        }
      }else{#length(x)>1
        # re-run wilcoxon.cut.test
        foreach(i = x, .combine=rbind)%dopar%{
          wilcoxon.cut.test(x=i, group=group, data=data, 
