...
#' valreg(reg,analyse=TRUE,boot=TRUE)
#' #Example 4
#' data(iris)
#' reg <- lm(Petal.Width~Petal.Length,data=iris[iris$Species=="virginica",])
#' valreg(reg,analyse=TRUE,boot=TRUE,plot=TRUE)
#' plot(reg)
valreg <- function(reg,analyse=FALSE,nvar=5,boot=FALSE,pval=0.05,conf.level=0.95,
                   plot=FALSE,
  ...
  if (nvar1 > nvar) {if(analyse==TRUE){cat("Plus de 6 variables\n")};error = FALSE
    ...
    if (pval_mdl > pval) {if(analyse==TRUE){cat("Bad significance of the model.\n")};error = FALSE}
    pval_coeff <- summary(reg)[[4]][,4]
    if (max(pval_coeff) > pval) {if(analyse==TRUE){cat("Bad significance of the coefficients.\n")};error = FALSE}
    raintest(reg)$p.value -> pvalt # test de rainbow Adequacy
    if (pvalt < raintest_pval) {if(analyse==TRUE){cat("(Rain test) Bad adequacy. p.value : ",pvalt,"\n")};error = FALSE}
    dwtest(reg)$p.value -> pvalt # Independence of DurbinWatson residuals
    if (pvalt < dwtest_pval) {if(analyse==TRUE){cat("(DurbinWatson test) Bad independence of the residuals. p.value : ",pvalt,"\n")};error = FALSE}
    shapiro.test(residuals(reg))$p.value->pvalt # Normal distribution of residues
    if (pvalt < shapiro_pval) {if(analyse==TRUE){cat("(Shapiro-Wilk test) Non-normal distribution of residuals. p.value : ",pvalt,"\n")};error = FALSE}
    if (length(coef(reg))>=2) {
      bptest(reg)$p.value -> pvalt # Breush Pagan: constant variance of residuals
      if (pvalt < bptest_pval) {if(analyse==TRUE){cat("(BreushtPagan test) Non-constant variance of the residuals. p.value : ",pvalt,"\n")};error = FALSE}
    }
    cooks.distance(reg)->cooksd
    if (max(cooksd,na.rm=TRUE) > 1) {if(analyse==TRUE){cat("(Cook's distance) Leverage effect.\n")};error = FALSE}
    if (boot == TRUE) {
      bootreg(reg,analyse=FALSE,pval=pval,conf.level=conf.level,plot=plot) -> bootres
      if (bootres==FALSE) {if(analyse==TRUE){cat("Fragility of the model in boostrap. Please, use bootreg()\n")};error = FALSE}
    }
  }
  return(error)
}
