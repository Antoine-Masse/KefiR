...
source("C:/Users/masse/Desktop/KefiR/KefiR/R/.dbg.R")
source("C:/Users/masse/Desktop/KefiR/KefiR/R/.strip_data_dollar_safe.R")
source("C:/Users/masse/Desktop/KefiR/KefiR/R/.vbse.R")
source("C:/Users/masse/Desktop/KefiR/KefiR/R/.format_pval.R")
source("C:/Users/masse/Desktop/KefiR/KefiR/R/.posthoc.R")
source("C:/Users/masse/Desktop/KefiR/KefiR/R/pairwise.R")
source("C:/Users/masse/Desktop/KefiR/KefiR/R/pairwise.boot.R")
source("C:/Users/masse/Desktop/KefiR/KefiR/R/catego.R")
...
VecteurB <- round(data$B/2)
m.test(VecteurB ~F, data)
VecteurC <- round(data$C/3)
m.test(VecteurC ~F, data)
...
.formulator(data$B~data$F)
# Exemple avec un ANOVA simple
f1 <- Y ~ x1 * x2
formulator(f1, mode = "aov")

# Exemple avec Friedman
f2 <- Y ~ x
formulator(f2, mode = "friedman")

# Exemple avec ANOVA permutée
f3 <- Y ~ x1 + x2
formulator(f3, mode = "aovp")

# Exemple avec des colonnes de données
f4 <- iris[,1] ~ iris[,5]
formulator(f4, mode = "aov")

# Exemple avec des colonnes de données
f5 <- iris[,1] ~ Species
formulator(f5, mode = "aov")

# Exemple avec des colonnes de données
f6 <- Note~Lycée+Error(établissement)
formulator(f6, mode = "aov")

# Exemple avec des colonnes de données
f7 <- Note~Lycée+Error(établissement)
formulator(f7, mode = "aov")
formulator(f7, mode = "aovp")
formulator(f7, mode = "friedman")

# Exemple avec des colonnes de données
f8 <- Note~Lycée|Sexe+Options
formulator(f8, mode = "aov") # J'attends un retour Note ~ Sexe + Options + Lycée
#formulator(f7, mode = "aovp")
#formulator(f7, mode = "friedman")

#
f9 <- response ~ factor1 * factor2 + (1|random_effect)
formulator(f9, mode = "aov", debug=FALSE) # J'attends un retour response ~ factor1 * factor2 + Error(random_effect)
#
f10 <- response ~ factor1 * factor2 + (factor1|random_effect) # J'attends un retour response ~ factor1 * factor2 + Error(random_effect/factor1)
formulator(f10, mode = "aov", debug=FALSE) # J'attends
#
f11 <- response ~ factor1 + factor2 + (factor1|random_effect)
formulator(f11, mode = "aov", debug=FALSE) # J'attends


# Exemple d'utilisation :
# Supposons que n = 20 (doit être divisible par 4, ici 20 = 4*5)
# result <- simul(20)
# head(result)


#######################
# Batteries de m.test() sur 3 catégories, prévoir plus...
#######################
...
data(iris)
m.test(iris[,1]~iris[,5], debug=TRUE)-> b

m.test(iris[,1]~iris[,5],debug=T)-> b
paris <- c(sample(1:50),sample(1:50),sample(1:50))
m.test(iris[,1]~iris[,5],id=paris)-> b
m.test(iris[,1]~iris[,5],id=paris, debug=TRUE)-> b

m.test(iris[,1]~iris[,5]+paris)-> b
m.test(iris[,1]~iris[,5]+paris,debug=TRUE)-> b


m.test(Sepal.Length~Species, data=iris, debug=TRUE)-> b
m.test(Sepal.Length~iris$Species, data=iris)-> b
m.test(iris[,1]~iris$Species, data=iris)-> b

factori <- iris[,4] ; ifelse(factori>2,"b","a")->factori
m.test(iris[,1]~iris$Species+factori, data=iris)-> b

paris <- sample(c(1:75,1:75))
m.test(iris[,1]~iris[,5],debug=T,pair=paris)-> b

dt <- data.frame(iris,"print"=paris)
m.test(dt[,1]~dt[,5],debug=T,pair='print')-> b




mf <- model.frame(iris[,1]~iris[,5], data = iris, na.action = na.omit)

m.test(iris[,2]~iris[,5])
m.test(iris[,3]~iris[,5])
m.test(iris[,4]~iris[,5])->b

n<-15
x <- rnorm(n*5,c(1:5),c(1:5))
ct <- rep(c("A","B","C","D","E"),each=n)
#FSA::dunnTest(x ~ ct, method = "holm")

m.test(x,ct,verbose=TRUE,code=FALSE)

m.test(x,ct,verbose=TRUE,code=TRUE)


m.test(Sepal.Length~Species+Sepal.Width, iris, debug=TRUE)
...
# Débugage
#########################
m.test(iris[,2]~iris[,5],debug=TRUE)->b
m.test(iris[,2]~iris[,5],debug=FALSE)->b
m.test(iris[,2]~iris[,5],debug=TRUE)->b

paris <- sample(c(1:75,1:75))

paris <- c(sample(c(1:25,1:25)),sample(c(1:25,1:25)),sample(c(1:25,1:25)))
# 2 catégories
#   formula
m.test(iris[1:100,1]~iris[1:100,5],debug=T,pair=paris[1:100])-> b
#   no formula
m.test(iris[1:100,1],iris[1:100,5],debug=T,pair=paris[1:100])-> b
