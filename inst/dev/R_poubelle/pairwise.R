...
#'   - "ks" for ks.test(),
#'   - "lincon" for lincon() from the {WRS2} package,
#'   - "dunn" for dunnTest() from the {FSA} package,
#'   - "BM" for brunner.munzel.test() from the {lawstat} package (independent samples),
#'   - "neminye" for frdAllPairsNemenyiTest() from the {PMCMRplus} package,
...
#' @param debug when pairwise returns an error.
#'
#' @return This function automates the work of the ks.test(), lincon() functions of {WRS2}, brunner.munzel.test() from {lawstat}, pairwise.t.test(), pairwise.wilcox.test(), dunnTest() from {FSA}, and frdAllPairsNemenyiTest() from {PMCMRplus}, and extracts groups of means or comparisons to a control with the catego() function.
...
pairwise <- function(x,g,type="mean",alpha=0.05,control=c(),pool.sd=FALSE,silent=TRUE,
		boot=FALSE,boot_type=mean,tr=0.2,
		iter=500,conf=0.95,paired=FALSE,debug=FALSE) {
...
print("après bootstrap")
	...
	#----------------------------------------
		...
		g <- reorder_groups(x, g, ind_control,get(boot_type))
