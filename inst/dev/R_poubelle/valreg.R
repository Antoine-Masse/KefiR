  ...
  if (nvar1 > nvar) {
    ...
    if (inherits(reg, "lm")) {
      ...
      if (nvar2 > 1) {
        ...
        if (pvalt < raintest_alpha) {
          if (verbose) cat(.msg("\tWarning!\n\tRainbow test ordered by mahalanobis (raintest()) - Bad adequacy. p.value:", "\tAttention !\n\tTest de Rainbow ordonné par mahalanobis (raintest()  de {lmtest}) - Mauvaise adéquation. p.value :"), pvalt, "\n")
          error <- FALSE
        } else {
          if (verbose) cat(.msg("\tRainbow test (raintest()) - Good adequacy. p.value:", "\tTest de Rainbow (raintest()  de {lmtest}) - Bonne adéquation. p.value :"), pvalt, "\n")
        }
      } else {
        if (length(data) == 0) {
        ...
        if (pvalt < raintest_alpha) {
          if (verbose) cat(.msg("\tWarning!\n\tRainbow test ordered by X (raintest()) - Bad adequacy. p.value:", "\tAttention !\n\tTest de Rainbow ordonné par X (raintest() de {lmtest}) - Mauvaise adéquation. p.value :"), pvalt, "\n")
          error <- FALSE
        } else {
          if (verbose) cat(.msg("\tRainbow test (raintest()) - Good adequacy. p.value:", "\tTest de Rainbow (raintest() de {lmtest}) - Bonne adéquation. p.value :"), pvalt, "\n")
        }
    ...
    if (length(coef(reg)) >= 2 & length(resid(reg)) <= 1000) {
      ...
      if (pvalt < bptest_alpha) {
		...
		if (inherits(reg, "lmerMod")) {
			if (verbose) cat(.msg("\tHeteroscedasticity is not always an issue in a mixed model. See {nlme}.\n","\tL'hétéroscédasticité n'est pas toujours un problème sur un modèle mixte. Voyez {nlme}.\n"))
		}
