...
.posthoc_ANCOVA <- function(ancova_model,
  ...
  # NOTE PERSO: Cette fonction implemente les post-hocs appropries pour ANCOVA
  # selon les recommandations academiques (Maxwell & Delaney, 2018; Huitema, 2011)
  # Format de sortie compatible avec .posthoc() pour integration dans m.test()
  ...
  # Verifier que le modele est bien un objet aov ou lm
  ...
  # Verifier disponibilite du package emmeans
  ...
  for (term in term_labels) {
    # Verifier si c'est un facteur ou une covariable
    if (term %in% names(model_data)) {
      # IMPORTANT: aov() traite automatiquement character comme factor
      # Donc on doit detecter BOTH factor ET character comme facteurs categoriques
  ...
  # Verifier qu'il y a au moins 1 facteur et 1 covariable
  ...
  # Determiner le facteur a analyser
  ...
  # ============================================================================
  # BLOC 3: CALCUL DES MOYENNES AJUSTEES (EMMs) - Fusionne avec affichage
  # ============================================================================

  if (isTRUE(code)) {
    cat("# ==================================================\n")
    cat("# Post-hocs ANCOVA : Moyennes marginales estimees\n")
    cat("# ==================================================\n\n")
    cat("# Calculer les moyennes ajustees (EMMs) aux moyennes des covariables\n")
    cat("library(emmeans)\n")
  ...
  # Extraire moyennes ajustees - AFFICHAGE FUSIONNE avec etape precedente
  ...
  # Affichage integre avec ajout "Moyennes ajustees:"
  if (verbose) {
  ...
  if (isTRUE(code)) {
    cat("# Comparaisons par paires des moyennes ajustees\n")
  ...
  # Extraire resultats des comparaisons
  ...
  # Affichage avec analyse des differences significatives
  if (verbose) {
    cat("\n\tComparaisons par paires (ajustement ", emmeans_method, ") :\n", sep = "")
    print(pairwise_comparisons, row.names = FALSE)

    # Conclusion : identifier les differences significatives
  ...
  # Utiliser multcomp::cld() pour generer compact letter display
  cld_result <- tryCatch({
    multcomp::cld(emm_result, alpha = alpha, Letters = letters, adjust = emmeans_method)
  }, error = function(e) {
  ...
  if (!is.null(cld_result)) {
    ...
    # Fallback: generer lettres basees sur p-values
  ...
  # ETAPE 13 SUPPRIMEE : L'affichage compact par lettres est maintenant
  # seulement disponible via la sortie de m.test() (synth$groups)
  # pour eviter redondance dans la sortie verbose

  # ============================================================================
  # BLOC 6: CREATION MATRICE P-VALUES
  # ============================================================================

  # Creer matrice de p-values pour compatibilite avec .posthoc()
  ...
  # Structure STANDARD harmonisee (meme ordre que ANOVA)
  # Elements obligatoires : groups, p.value, global_pvalue, descriptive_stats, method
  # Elements specifiques ANCOVA stockes dans $details (pour utilisateurs avances)

  synth <- list(
    # --- Elements STANDARD (harmonises avec ANOVA) ---
    groups = groups_df,
    p.value = p_value_matrix,
    # global_pvalue sera ajoute par m.test()
    # descriptive_stats sera ajoute par m.test() depuis adjusted_means
    method = paste0("ANCOVA post-hoc (", method, " adjustment)"),

    # --- Elements specifiques ANCOVA (pour compatibilite/details) ---
    adjusted_means = adjusted_means,  # Utilise pour construire descriptive_stats
    details = list(
