#' Analyse modeles mixtes (Linear Mixed-Effects Models)
#'
#' Fonction interne pour analyser donnees avec effets fixes et aleatoires.
#' Utilisee pour designs complexes: mesures repetees desequilibrees, effets
#' clusters, structures imbriquees/croisees.
#'
#' @param x Vecteur numerique - Variable dependante
#' @param g Facteur ou data.frame - Variable(s) groupante(s)
#' @param formula Formule (optionnel) - Peut inclure effets aleatoires
#' @param data Data.frame (optionnel) - Donnees source
#' @param paired Logique - Mesures repetees/appariees
#' @param id Caractere - Nom colonne identifiant sujet (si paired=TRUE)
#' @param within Vecteur caracteres - Facteurs intra-sujets
#' @param between Vecteur caracteres - Facteurs inter-sujets
#' @param random_slope Logique - Inclure pente aleatoire (defaut FALSE)
#' @param alpha Numerique - Seuil significativite (defaut 0.05)
#' @param k Entier - Compteur messages verbose
#' @param code Logique - Afficher code R reproductible
#' @param debug Logique - Messages debogage detailles
#' @param verbose Logique - Messages explicatifs pedagogiques
#'
#' @details
#' Cette fonction implemente les recommandations academiques pour modeles mixtes:
#'
#' 1. DETECTION AUTOMATIQUE:
#'    - Effets aleatoires depuis formule ou parametres (id, within, between)
#'    - Type structure: imbriquee vs croisee
#'    - Necessite pente aleatoire (random slopes)
#'
#' 2. CONSTRUCTION FORMULE:
#'    - Intercept aleatoire: (1 | id)
#'    - Pente aleatoire: (predictor | id)
#'    - Effets croises: (1 | factor1) + (1 | factor2)
#'
#' 3. VALIDATION MODELE:
...
#'
#' @return Liste avec elements:
#'   - model: Objet lmerMod (modele ajuste)
#'   - formula: Formule utilisee
#'   - validation: Resultats valreg()
#'   - anova_table: Tableau ANOVA type III
#'   - variance_components: Composantes variance (VarCorr)
#'   - random_effects: Effets aleatoires par groupe (ranef)
#'   - fixed_effects: Effets fixes (fixef)
#'   - global_pvalue: P-value globale pour return=FALSE
#'   - k: Compteur messages mis a jour
#'   - assumptions_checked: Liste diagnostics
#'   - warnings: Vecteur avertissements
#'
#' @note
#' Fonction interne - Non exportee
#' Utilisee par m.test() via .multi_factor_analysis()
#'
#' @references
#' Bates, D., M?chler, M., Bolker, B., & Walker, S. (2015).
...
.mixed_model_analysis <- function(x = NULL,
  ...
  if (isTRUE(code)) {
    ...
    verbose <- FALSE
    k_code <- 0  # Compteur separe pour mode code
  }
  ...
  ################################################################################
  # AJUSTEMENT MODELE MIXTE
  ################################################################################
  # References academiques (commentaires code):
  ...
  if (has_formula_random) {
    # Formule contient deja effets aleatoires
    lmer_formula <- formula

  } else if (!is.null(id)) {
    # Construire formule depuis parametres
    ...
    if (length(fixed_terms) > 0) {
      ...
      fixed_part <- "1"  # Modele intercept-only
    }

    # Construire partie aleatoire
    random_model_type <- ""
    if (random_slope && length(within) > 0) {
      # Pente aleatoire sur premier facteur within
      random_part <- paste0("(", within[1], " | ", id, ")")
      random_model_type <- "random slopes"
    } else {
      # Intercept aleatoire seulement
    ...
    # Pas d'effets aleatoires identifies
    k <- .vbse(
  ...
  # MESSAGE UNIQUE: Ajustement modele avec toutes les infos
  k <- .vbse(
...
#return(mixed_model)
  ...
  # NOTE: Ne PAS retourner directement le modele ici - continuer pour valreg() et construction bilan
  # return(mixed_model)  # LIGNE DESACTIVEE - causait erreur "bilan[[4]] : indice hors limites"
  # Appel valreg() - son bilan s'affiche directement avec numerotation continue
  valreg_output <- tryCatch({
    valreg(
      reg = mixed_model,
      verbose = verbose, code = code,  # CRITIQUE: doit etre TRUE pour afficher
      alpha = alpha,
      boot = FALSE,
      plot = FALSE,
      k = k,
  ...
  if (!isTRUE(validation_result)) {
    ...
    # Reference: Bolker et al. (2009) Generalized linear mixed models: a practical guide
    k <- .vbse(
  ...
  ################################################################################
  # BLOC 5: RESULTATS STATISTIQUES DU MODELE MIXTE
  ################################################################################

  # ANOVA Type III avec stats::anova() (methode lmerTest pour approximation Satterthwaite)
  ...
  # Identifier effets significatifs pour interpretation
  ...
  if (verbose) {
    ...
    # INTERPRETATION immediate apres le tableau ANOVA
  ...
  # Convertir en dataframe pour eviter erreurs S4 as.vector()
  ...
  if (verbose) {
    ...
    cat("\n")
    print(var_comp_S4)  # Afficher format original pour lisibilite
    cat("\n")
  }
  ...
  # Effets aleatoires par groupe avec lme4::ranef()
